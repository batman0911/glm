...
```{r}
library(ggplot2)
data("belection"); str(belection)

sumF <- sum(belection$Females)
#belection$Party

#plot(belection$Party, belection$Females)
sum.f <- c(sum(belection[belection$Party == "Cons",]$Females),
  sum(belection[belection$Party == "Labour",]$Females),
  sum(belection[belection$Party == "LibDem",]$Females),
  sum(belection[belection$Party == "Green",]$Females),
  sum(belection[belection$Party == "Other",]$Females))
Party <- c("Cons", "Labour", "Lib-Dem", "Greens", "Other")
df.f <- data.frame(sum.f, Party)
df.f
qplot(df.f$Party, df.f$sum.f)
```

```{r}
# fit model
fit <- glm(Females / sumF ~ Party + Region, weights = rep(sumF, length(Females)), data = belection, family = binomial())
summary(fit)
```
ex 9.11
```{r}
